...
class MyModel(nn.Module):
    def __init__(self, prop_steps, feat_dim, hidden_dim, output_dim, threshold, r=0.5):
        ...
        self.NonPLP = NonParaLP(prop_steps=10, num_class=self.output_dim, alpha=0.5)
    ...
    def non_para_lp(self, subgraph, nodes_embedding, x, device):
        ...
        if self.reliability_acc >= self.threshold:
            self.homo = True
            self.homo_init()
